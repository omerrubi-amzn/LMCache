...
The underlying thread-worker pool (``_ThreadWorkerPool``) now lives in the
shared module ``glide_pool`` so it can be reused by the MP-mode L2 adapter
(``distributed/l2_adapters/glide_batch_client.py``). This module keeps the
non-MP ``RemoteConnector`` implementation and re-exports the pool symbols for
backward compatibility (``valkey_adapter.py`` and existing tests import the
constants from here).
...
# Shared GLIDE pool (extracted). Re-exported for backward compatibility:
# ``valkey_adapter.py`` and existing tests import these names from here.
from lmcache.v1.storage_backend.connector.glide_pool import (
